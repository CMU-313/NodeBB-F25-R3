        ...
        if (username.length < ajaxify.data.minimumUsernameLength ||
            ...
            Promise.allSettled([
                ...
                if (results.every(obj => obj.status === 'rejected')) {
                    ...
                    // Make a username suggestion by adding suffix number
                    const recommendation = username.concat(Math.random().toString(36).substring(2, 7)).concat('<br />');
                    showError(username_notify, `[[error:username-taken]]. Try ${recommendation} instead.`);
